...
export default async function BocceDashboard() {
  const { playerCount, teamCount, activeSeason, upcomingGames, recentGames } = await getDashboardData();

  const stats = [
    { label: 'Active Players', value: playerCount, href: '/bocce/players', color: 'bg-primary' },
    { label: 'Teams', value: teamCount, href: '/bocce/teams', color: 'bg-secondary' },
    { label: 'Divisions', value: activeSeason?.divisions.length || 0, href: '/bocce/divisions', color: 'bg-primary-light' },
    { label: 'Season', value: activeSeason?.name || 'None', href: '/bocce/settings', color: 'bg-accent' },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Back to Club Portal */}
      <div className="mb-4">
        <Link href="/" className="inline-flex items-center text-sm text-muted hover:text-primary transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Upcoming Games */}
        <Card title="Upcoming Games" action={<Link href="/bocce/games" className="text-sm text-primary hover:underline">View All</Link>}>
        ...
        {/* Recent Results */}
        <Card title="Recent Results" action={<Link href="/bocce/games" className="text-sm text-primary hover:underline">View All</Link>}>
